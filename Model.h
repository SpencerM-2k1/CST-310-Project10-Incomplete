...
public:
	...
	//HACK: The sphere and cylinder have no textures in their meshes normally.
	//There's also no clear way to pass in texture ID's, etc. ...
	//Use this function in order to do so. It only works if there's only one mesh, but it'll do for now.
	void assignTexture(GLint textureID)
	{
		std::string type = "";
		aiString path;
